...
- #### ```_errorHandler```
  ...
  **异常处理器**是一个参数列表为```(errType, errDetail)```的```Function```：
  ...
  - **```errDetail```**：异常的细节信息，为```Object```。**基础输出器**中异常的细节信息结构为：```{ error }```。
    ...
    ::: tip 提示
    我们可能遇到一些需要自定义异常文案的场景，此时可以通过在这些自定义```Error```产生之前修改```Core.Messages```中对应宏变量以实现对自定义```Error```文案的变更。
    :::
...
- #### ```sourcePath```
  ...
  **归档源目录**是**日期输出器**进行日志文件分割/清理的工作区，我们将在[日志分割](#日志分割)和[日志清理](#日志清理)一节中进行详细讨论。
...
2. 解析每个**日志文件名**中的**归档偏移**，并尝试读取**归档偏移**最大的**日志文件**。

3. 根据最后一个偏移处**日志文件**的体积计算新的**归档偏移**决定接下来输出的目标**日志文件**，并缓存目标**日志文件**的体积。

#### 在日期输出器执行输出时：

1. 计算输出动作造成的体积增量，并更新至缓存中的**日志文件**体积。

2. 比较缓存中的**日志文件**体积与分割阈值```maxSize```的关系，决定输出目标**日志文件**。
...
向**清理引擎**挂载清理规则需要指定**目标清理目录**、**项目选中规则**、**日期提取规则**和**保留日期数量**，**清理引擎**将按照配置进行定向清理：

1. **清理引擎**将读取并迭代**目标清理目录**中的文件和目录名称，并筛选出符合**项目选中规则**的文件/目录名称。
2. 迭代符合**项目选中规则**的文件/目录名称，使用**日期提取规则**提取其**归档日期**，最终得到源**归档日期**列表。
...
- 解析**日志文件名**获取**归档日期**，选择**归档前缀**与当前**日期输出器**一致且**归档日期**小于当前日期的文件。
...
| 异常场景                               | 异常类型（```errType```）归属             | 导致异常的```Error```                         |
| :----------------------------------- | :-------------------------------------- | :------------------------------------------- |
...
**文件输出器**支持使用**自动模式**和**手动模式**管理状态和资源，默认使用**自动模式**。当然，我们可以在实例化**文件输出器**时调整```auto```配置项以选择**文件输出器**的**运行模式**。

#### 自动模式

使用**自动模式**时，**文件输出器**将自动完成状态和资源管理。我们在**自动模式**下无需显式调用```start()```和```close()```，在期望输出日志时直接调用```log()```即可。

::: tip 提示
使用**自动模式**时，我们通常配合```timeout```设置**文件输出器**资源控制的懒惰程度。
:::

**文件输出器**将会自动在合适的时机进行状态和资源管理：

- 执行日志输出动作时，自动检测当前状态。若处于关闭状态，**文件输出器**将自动变更输出器为启动状态并创建相关资源。
- 如果**文件输出器**在```timeout```指定的时间内没有日志输出动作，将自动变更**文件输出器**为关闭状态并释放其占用的资源（比如：文件句柄等）。

#### 手动模式

使用**手动模式**时，**文件输出器**不会自动管理状态和资源，需要业务层手动执行相关控制逻辑。

我们需要知道的是：

- 执行```start()```将变更**文件输出器**为启动状态并创建相关资源。
- 执行```close()```将变更**文件输出器**为关闭状态并释放占用资源。
- **文件输出器**仅允许在启动状态才能使用```log()```输出日志。

因此，我们在期望输出日志前需要显式执行```start()```启动**文件输出器**；并在不需要使用输出器时使用```close()```解除**文件输出器**的资源占用。

### 输出规则

**文件输出器**允许在实例化时调整配置参数以指定日志的输出规则。
...
**文件输出器**提供了通过**归档前缀**和**归档日期**对**日志文件**进行分类的能力。我们可以通过实例化**文件输出器**时调整配置参数以指定是否附加**归档前缀**或**归档日期**至**归档源目录**构成最终的**日志输出目录**。

::: warning 注意
出于**日志文件**检索的复杂性考虑，我们无法直接附加**归档日期**至**归档源目录**得到**日志输出目录**。

即：仅在使用**归档前缀**预分类**日志文件**时，才能通过**归档日期**对**日志文件**进行二次分类。

我们将在[配置联动](#配置联动-2)一节中详细讨论配置间的依赖关系。
:::
...
**日志文件名**结构为：**<%归档前缀%>.<%归档文件名%>.<%归档日期%>.<%归档类型%>.<%文件后缀%>**。
...
- #### ```sourcePath```
  ...
  - 当由**归档源目录**和**归档前缀**或**归档日期**构成的**日志输出目录**在文件系统上不存在时，**文件输出器**将在实例化过程中尝试进行创建。如果**日志输出目录**创建失败将导致运行环境降级。

  :::

  ---

  通常，我们的应用程序中所有**日志输出器**应该使用同一个**归档源目录**以实现日志的集中存储：
...
- #### ```auto```
  ...
  设置此配置将指定**文件输出器**的**运行模式**，默认值为```true```。

- #### ```timeout```

  设置此配置将指定**文件输出器**在**自动模式**中自动释放资源的等待时间，单位为毫秒，默认值为```10000```。

  ::: tip 提示
  此配置仅在**文件输出器**运行在**自动模式**时生效，**手动模式**下将被锁定为```0```。
  :::

---

我们在使用**自动模式**的**文件输出器**时，无需显式调用```start()```和```close()```。**文件输出器**将在**自动模式**下强行调用```start()```和```close()```时抛出异常，我们可以通过在实例化时配置```_errorHandler```进行捕获：
...
而在**手动模式**下，如果没有显式调用```start()```和```close()```变更**文件输出器**状态强行使用```log()```发起日志输出动作同样会抛出异常：
...
- #### ```filePrefixAsFileName```
  ...
  设置此配置将指定**文件输出器**是否附加**归档前缀**至**日志文件名**，默认值为```true```。
...
- #### ```keepDateNum```
  ...
  需要注意的是，**文件输出器**将保留包含当前归档周期在内的最近```keepDateNum```个周期内生成的**日志文件**。
...
接下来，我们再来分析**归档日期格式**的联动关系。**归档日期格式**的联动关系与**归档前缀**类似：

1. 如果**归档日期格式**无效，则**归档日期**无法参与**日志输出目录**和**日志输出文件**的构建。
2. 如果**归档日期格式**有效，则**归档日期**至少参与**日志输出目录**和**日志输出文件**其中一项的构建。否则，设置**归档日期格式**没有意义。

另外，我们已经知道，只有在**使用归档前缀构建日志输出目录**时才能启用**使用归档日期构建日志输出目录**。因此：
...
**文件输出器**将在配置参数违反联动关系时自动校正。

对于**归档前缀**的联动关系，**文件输出器**将确保：

1. 当```filePrefix```无效时，将锁定```filePrefixAsSourcePath```和```filePrefixAsFileName```为```false```。
2. 当```filePrefix```有效时，则```filePrefixAsSourcePath```和```filePrefixAsFileName```中必须至少有一项指定为```true```。
3. 当```filePrefix```有效时，若```filePrefixAsSourcePath```和```filePrefixAsFileName```都配置为```false```，输出器将锁定```filePrefixAsSourcePath```为```true```。即**文件输出器**将在违反**归档前缀**联动关系时，优先使用**归档前缀**构建**日志输出目录**而不是用于构建**日志输出文件**。

对于**归档日期**的联动关系，**文件输出器**将确保：

1. 当```dateFormat```无效时，将锁定```dateAsSourcePath```和```dateAsFileName```为```false```。
2. 当```dateFormat```有效时，则```dateAsSourcePath```和```dateAsFileName```中必须至少有一项指定为```true```。
3. 当```dateFormat```有效时，若```dateAsSourcePath```和```dateAsFileName```都配置为```false```，输出器将锁定```dateAsSourcePath```为```true```。即**文件输出器**将在违反**归档日期**联动关系时优先使用**归档日期**构建**日志输出目录**而不是用于构建**日志输出文件**。

对于**归档前缀**和**归档日期**的高阶联动，**文件输出器**将确保：

1. 当```filePrefix```无效时，将锁定```dateAsSourcePath```为```false```。
2. 当```filePrefix```有效，且```filePrefixAsSourcePath```为```false```时，将锁定```dateAsSourcePath```为```false```。
3. 当```dateFormat```有效，且```dateAsSourcePath```因①或②而被锁定为```false```时，则锁定```dateAsFileName```为```true```。

此时，既能够保证**归档前缀**和**归档日期**的高阶联动性，又可以保证指定的**归档日期格式**将发挥作用。

### 日志清理

**文件输出器**同样使用**清理引擎**实现日志清理。我们通过指定```keepDateNum```配置为```>0```的值启动**文件输出器**的自动清理功能。

清理检索阶段**文件输出器**同样是在**归档源目录**中寻找**归档前缀**相同的**日志文件**。因此，我们可以通过指定```filePrefix```和```filePrefixAsSourcePath```配置，使用有效的**归档前缀**构建**日期输出目录**以将提升清理检索的效率。

关于**清理引擎**的运行原理，可以参照[日期输出器](#日期输出器)的[日志清理](#日志清理)一节。
...
- 当未使用**归档前缀**预分类**日志文件**时，选择**归档前缀**与当前**日期输出器**一致且**归档日期**小于当前日期的文件。
- 当使用了**归档前缀**预分类**日志文件**时：
  - 如果未使用**归档日期**分类**日志文件**，此时提取源为**日志文件名**，解析**日志文件名**获取**归档日期**，选择**归档日期**符合**日期输出器**指定**归档日期格式**的**日志文件**。
  - 如果使用了**归档日期**分类**日志文件**，此时提取源为以**归档日期**创建的目录名，选择**归档日期**符合**日期输出器**指定**归档日期格式**的**日志目录**。

**日期提取规则**：

- 当未使用**归档前缀**预分类**日志文件**时，解析**日志文件名**得到的**归档日期**。
- 当使用了**归档前缀**预分类**日志文件**时：
  - 如果未使用**归档日期**分类**日志文件**，此时提取源为**日志文件名**，则解析**日志文件名**得到的**归档日期**。
  - 如果使用了**归档日期**分类**日志文件**，此时提取源为以**归档日期**创建的目录名，直接作为**归档日期**。
