...
**日期输出器**在非```BASE_LOGGER_DEVELOPMENT_ENVIRONMENT```环境中，将同一周期内产生的日志输出至文件，我们可以选择将这些日志归档至单个文件或是拆分成多个文件。同时，我们还可以指定保留日志周期的个数，**日期输出器**会帮我们自动清理过期的日志文件。
...
**日期输出器**对异常处理器支持的异常类型```errType```进行了拓展：
...
| 异常描述宏                                     | 产生时机                     |
| :------------------------------------------- | :-------------------------- |
| ```DATE_LOGGER_MESSAGE_CANT_CREATE_FILE```   | 日期输出器无法创建日志输出文件时 |
| ```DATE_LOGGER_MESSAGE_CANT_CREATE_FOLDER``` | 日期输出器无法创建日志输出目录时 |
