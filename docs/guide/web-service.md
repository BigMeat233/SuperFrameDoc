...
通过创建Corejs的Web服务核心组件ServiceCore的实例启动Web服务，通过调整ServiceCore实例化配置控制Web服务运行参数。
...
在创建ServiceCore实例时，如果指定驻留端口为443，则必须在```ssl```配置项中设置TLS/SSL密钥和证书，否则将无法成功启动ServiceCore：
...
ServiceCore在启动过程中默认使用node原生的```http.createServer()```或```https.createServer()```创建最终承载Web服务的Server实例。如果有特殊场景需要变更Server实例的构建过程，可以在**执行启动前**对ServiceCore实例中的```createServer```属性进行更改以达到自定义Server构建过程的目的。

::: danger 注意
自定义Server构建过程时，```createServer```方法中需要包含Server实例构建和启动逻辑。
:::

样例将以默认Server构建行为演示如何变更Server构建过程：
...
**ServiceCore拦截器为全局拦截器，在ServiceCore启动时，封装拦截器为Express标准中间件挂载至Express中间件列表的最前端以实现拦截全部用户请求功能**，因此接收到用户请求时，可以在全局拦截器中决定是否放行此次请求进入下游处理。若在全局拦截器执行过程中产生了未捕获的异常，将进入[错误拦截器](#错误拦截器)处理。

::: danger 注意
默认行为下，如果用户请求路径未能匹配到任何一个Handler，则认为请求无效，直接返回404状态码，不再进入后续全局中间件和Handler处理流程。

所以，如果在ServiceCore的全局中间件中使用了静态资源中间件（如：```express.static```），在默认全局拦截器行为下可能导致所有静态资源请求无效。这种场景下，需要**变更全局拦截器拦截逻辑，放行相关资源请求**或**在Handler维度使用静态资源中间件**。
:::

**在ServiceCore启动前**，可以对实例```globalInterceptor```属性进行更改以达到自定义全局拦截器的目的。

样例将以默认全局拦截器行为演示如何变更全局拦截器：
...
**注意：Handler维度可以根据用户请求路径、参数等动态挂载中间件列表，同样支持所有Express生态中间件**。全局中间件阶段结束后ServiceCore分发用户请求进入对应的Handler处理，即：全局中间件在Handler级别中间件之前执行。
...
**ServiceCore将全局错误拦截器封装为Express标准中间件挂载至Express中间件列表的最末端**，因此，错误拦截器与Express错误处理中间件表现相同：仅可捕获同步异常（全局拦截器执行阶段、全局中间件执行阶段），异步产生的异常需要进行特殊处理（可参考Express文档中对错误捕获的相关描述）。
...
ServiceCore针对特定路径的[请求处理](/guide/request-handler)场景需要配合Handler实现，Handler有独立的生命周期和处理流程。全局中间件处理结束后，用户请求进入与请求路径匹配的Handler执行后续处理。

使用继承的方式实现Handler，通过重写```static getRoutePath()```方法指定用户请求路径规则，符合规则的请求将进入Handler处理；处理特定请求方式的用户请求需要实现与请求方式对应的```Handler Method```，比如：需要处理用户POST方式请求，重写Handler中```postHandler(req, res, next)```即可。
...
ServiceCore支持通过外部注入日志输出器的方式进行日志收集，ServiceCore内部通过调用输出器的```log(level, funcName, message)```方法输出日志：

参数：

- ```level```：输出等级
  > 通过在实例化ServiceCore前修改宏的方式实现对默认日志输出等级的变更：
...
- ```message```：文案内容
  > 通过在实例化ServiceCore前修改宏的方式实现对默认日志输出文案的变更：
...
- ```funcName```：调用方法名
  > 通过在实例化ServiceCore前修改宏的方式实现对默认调用方法名的变更：
